...
class SaleIdImpl(ValueObject[UUID], SaleId):

    def __init__(self, value: UUID):
        super().__init__(value)

    @classmethod
    def generate(cls) -> "SaleIdImpl":
        try:
            return cls(uuid4())

        except DomainException as e:
            raise UnexpectedDomainException(
                f"Error al generar el ID de venta: {str(e)}",
                e,
            )

    @classmethod
    def from_string(cls, value: str) -> "SaleIdImpl":
        try:
            return cls(UUID(value))

        except ValueError:
            raise DomainException(
                f"El ID de venta no tiene un formato válido: {value}"
            )
